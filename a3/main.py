...
def buildTriangles( slice0, slice1 ):
    ...
    #
    # [1 mark]
    # a slice is collection of verticies
    # [YOUR CODE HERE]
    minLength=0
    closeVert0=0
    closeVert1=0
    ...
    idx1=0
    count0=0
    for vert0 in slice0.verts:
        count1=0
        for vert1 in slice1.verts:
            if minLength == 0:
                minLength=distance(vert0.coords,vert1.coords)
                idx1 = count1
                idx0 = count0
                closeVert0=vert0
                closeVert1=vert1
            elif distance(vert0.coords,vert1.coords)<minLength:
                minLength=distance(vert0.coords,vert1.coords)
                idx1=count1
                idx0=count0
                closeVert0 = vert0
    ...
    # [1 mark]
    # [YOUR CODE HERE]
    #go through the slice 0 array and make a cyclic permutation
    numVerts0=len(slice0.verts)
    tempVert=closeVert0
    newList0=[]
    tempidx0=idx0
    while True:
        newList0.append(slice0.verts[tempidx0])
        tempidx0 = (tempidx0 + 1) % numVerts0
        if tempidx0 == idx0:
            break
    newList0.append(slice0.verts[idx0])
    #do the same thing but with the slice1 array
    numVerts1 = len(slice1.verts)
    tempVert = closeVert1
    newList1=[]
    tempidx1=idx1
    while True:
        newList1.append(slice1.verts[tempidx1])
        tempidx1 = (tempidx1 + 1) % numVerts1
        if tempidx1 == idx1:
            break
    newList1.append(slice1.verts[idx1])
    ...
    #
    # [2 marks]
    #deal with calculating the triangle areas with these things?
    # [YOUR CODE HERE]
    runningSum = 0
    ...
    # [YOUR CODE HERE]
    runningSum
    tempVert0=closeVert0
    tempVert1=closeVert1
    for r in range(1,numVerts1+1):
        for c in range(1,numVerts0+1):
    ...
    while True:
        if minDir[r][c]==Dir.PREV_ROW:
            newTri=Triangle([newList1[r],newList0[c],newList1[r-1]])
            triangles.append(newTri)
            r=r-1
        else:
            newTri=Triangle([newList1[r],newList0[c],newList0[c-1]])
            triangles.append(newTri)
            c=c-1
        if r==0 and c==0:
            break
